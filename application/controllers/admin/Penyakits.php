...
class penyakits extends CI_Controller
{
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->db->set('namapenyakit', $nama);
            $this->db->set('jenis', $jenis);
            $this->db->set('nonkimiawi', $nkimia);
            $this->db->set('kimiawi', $kimia);
            $this->db->set('foto', $foto);
            $idpenyakit = $this->db->get_where('penyakit', ['id' => $id])->row_array();
            $idp = $idpenyakit['idpenyakit'];
            $config['upload_path'] = './assets/img/opt';
            $config['allowed_types'] = 'jpg|jpeg|png';
            // $config['file_name']            = $idp.'png';

            $this->load->library('upload', $config);

            if ($this->upload->do_upload('foto')) {
                $this->load->helper("file");
                delete_files($idpenyakit['foto']);
                $data = array('upload_data' => $this->upload->data());
                ...
                $this->session->set_flashdata('success', 'Data Penyakit No.' . $idpenyakit['idpenyakit'] . ' Telah Diupdate!');
